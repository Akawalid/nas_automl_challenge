...
def get_transforms(
    dataset_name: str, data_augmentation: list[str] | None = None, sample_data=None
) -> tuple[list, list]:
    datasets_transforms = {
        "mnist": [
        ...
        for aug in data_augmentation:
            if aug == "horizontal_flip":
                ...
                if dataset_name in ["cifar10", "cifar100"]:
                    ...
                    warn(f"AutoAugment not available for {dataset_name}, skipping")
                    continue
    ...
    # Fallback for unknown/unseen dataset codenames: compute normalization
    # stats directly from the data instead of a hardcoded lookup.
    if dataset_name not in datasets_transforms:
        if sample_data is not None:
            data = torch.as_tensor(sample_data, dtype=torch.float32)
            if data.max() > 1.0:
                data = data / 255.0
            dims = tuple(d for d in range(data.dim()) if d != 1)  # keep channel dim
            mean = data.mean(dim=dims).tolist()
            std = data.std(dim=dims).tolist()
            ...
            base_transforms = [Tv2.ToDtype(torch.float32, scale=True)]
...
def line_search(
    ...
    # gamma = t ** 2
    if t0 is None:
        t = np.sqrt(2 * (initial_loss / first_order_improvement)) if first_order_improvement > 0 else 0.0
    else:
        t = np.sqrt(t0)
