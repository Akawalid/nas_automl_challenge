...
# This is the Phase 3 copy of ingestion_program: it must match Phase 3's
# `execution_time_limit` in competition.yaml (86400s = 24h), which is why this
# default differs from the Phase 2 copy (ingestion_program/ingestion.py, 3600s).
# Codabench does not expose execution_time_limit to the running container, so
# this value has to be kept in sync by hand between the two copies.
TIME_LIMIT_SECONDS = int(os.environ.get("TIME_LIMIT_SECONDS", 86400))
