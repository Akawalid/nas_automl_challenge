...
class GrowingModule(torch.nn.Module):
    ...
    def extended_forward(
        ...
        # LOCAL PATCH (not upstream Santiago23florido/gromo@AblationDAG -- see
        # README_DEMETER.md "Local gromo patch"): for a brand-new node's own edge (not yet
        # apply_change()'d), self.layer is a real but genuinely 0-in_channels nn.Conv2d/Linear
        # (in_channels=0 is correct here -- there IS no pre-existing weight on an edge that
        # didn't exist a moment ago). PyTorch's degenerate 0-in-channel path computes the right
        # VALUE (zero, or bias-only where a bias exists -- verified against
        # GrowingDAG.add_node_with_two_edges, which explicitly disables bias on one of the two new
        # edges and leaves the other's default, so the two edges are NOT interchangeable and must
        # not be zeroed out uniformly) but returns it disconnected from the autograd graph (a leaf
        # with grad_fn=None), which crashes the very next in-place `+=`/`-=` on it. `.clone()` is
        # value-preserving (same number, same possible bias) and only fixes the autograd
        # connectivity, so it does not change what the network computes.
        if pre_activity.is_leaf:
            pre_activity = pre_activity.clone()
