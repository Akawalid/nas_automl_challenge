...
class GrowingDAG(nx.DiGraph, GrowingContainer):
    ...
    def extended_forward(
        ...
        for node in nx.topological_sort(self):
            ...
            # README_DEMETER.md "Local gromo patch"): output[node][1] (the extension tensor) is
            # legitimately None whenever nothing upstream of this node was in `mask` -- the normal
            # case for every DAG cell except the one actually being grown this round in a
            # multi-cell composite model. Calling merge_module(None) crashes inside
            # MergeGrowingModule.forward (`y.detach()` on None). Guarded the same way the
            # activity_ext accumulation two lines above already is.
            output[node] = (
